...
public class Show {
    ...
    //根据传入的Arraylist显示
    public static String[][] show2(ArrayList<Furniture> furs) {
        String[][] value  = new String[60][6];
        int i = 0;
        Connection connection = null;
        PreparedStatement preparedStatement = null;
        ...
        try {
            try {
                connection = DB.getConnection();
                String sql = "select * from Furnitures where id = ?;";

                for (int j = 0; j < furs.size(); j++) {
                    preparedStatement = connection.prepareStatement(sql);
                    preparedStatement.setInt(1, furs.get(i).getId());
                    resultSet = preparedStatement.executeQuery();
                    if (resultSet.next()) {
                        {   //找到的情况
                            do {
